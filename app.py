"""PDF RAG: local MiniLM embeddings + per-session FAISS + Groq answers.
...
SYSTEM_PROMPT = """You answer questions about an uploaded PDF.
...
def suggest_questions(pages):
    """Match common question templates to topics in the PDF, with no API call."""
    ...
    topics = [
        ...
        (r"\b(education|qualifications|degree)\b", "What qualifications are mentioned?"),
        (r"\b(skills|technical skills)\b", "What skills are listed?"),
        (r"\b(projects|project experience)\b", "What projects are described?"),
        (r"\b(methodology|research method|methods)\b", "What methods does the document describe?"),
        (r"\b(findings|results|conclusions)\b", "What findings are reported?"),
        (r"\b(eligibility|eligible)\b", "What are the eligibility requirements?"),
        (r"\b(deadline|closing date)\b", "What deadlines are mentioned?"),
    ]
    ...
    return questions[:6] or [
        ...
        "What key information can you find in the retrieved excerpts?",
    ]


def main():
    st.set_page_config(page_title="PDF Knowledge Assistant", page_icon="📄", layout="centered")
    st.title("PDF Knowledge Assistant")
    st.write("Upload your PDF, choose a suggested question, or ask your own.")
    st.caption("Best for English PDFs with selectable text. Each question is independent; include the subject in follow-up questions.")
    ...
    with st.sidebar:
        ...
        st.markdown("1. **Upload your PDF.** Use a file with selectable text.\n\n"
    ...
    uploaded = st.file_uploader(
        "Choose one PDF (up to 10 MB and 150 pages)", type=["pdf"],
    ...
    if uploaded is None:
        ...
        st.session_state.preparation_error = ""
        st.session_state.question_input = ""
        st.info("Start by uploading a PDF containing selectable text.")
        return
    ...
    if not bundle:
        ...
        if not st.session_state.preparation_attempted or retry:
            ...
            try:
                with st.spinner("Extracting text, downloading/loading the model and building your index…"):
    ...
    st.caption("Suggested from topics detected in your document—not measured user activity. Click a question to get an answer.")
    ...
    st.caption("Answers can be incomplete or incorrect. Verify important claims against the displayed PDF excerpts.")
